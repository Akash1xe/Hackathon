...
export default function DashboardOverview({ user }) {
  ...
  useEffect(() => { fetch('/api/profile').then((response) => response.ok ? response.json() : null).then(setProfile).catch(() => {}); }, []);
  ...
  return (
    <main className="site-shell py-10 sm:py-14">
      ...
      {profile?.reputation && <section className="surface mt-4 flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="eyebrow">Civic reputation</p><div className="mt-2 flex items-end gap-3"><strong className="text-3xl font-black">{profile.reputation.score || 0}</strong><span className="pb-1 text-sm font-extrabold capitalize text-[#0b6b58]">{(profile.reputation.tier || 'newcomer').replaceAll('_',' ')}</span></div></div><div className="grid grid-cols-3 gap-5 text-center text-sm"><div><strong className="block text-lg">{profile.reputation.validReports || 0}</strong><span className="text-xs text-[#627570]">Valid reports</span></div><div><strong className="block text-lg">{profile.reputation.resolvedReports || 0}</strong><span className="text-xs text-[#627570]">Resolved</span></div><div><strong className="block text-lg">{profile.reputation.communityValidations || 0}</strong><span className="text-xs text-[#627570]">Validations</span></div></div></section>}
